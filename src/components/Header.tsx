...
//         <>
//             <header
//                 className={`sticky top-0 left-0 w-full p-3 h-auto z-50 border-b-2 border-yellow-500 transition-all duration-300 ${isScrolled ? "bg-black/80 backdrop-blur-md shadow-lg" : "bg-black/95"
//                     }`}
//             >
//                 <div className="max-w-7xl py-2 mx-auto px-3 sm:px-4 lg:px-8">
...
const Header = () => {
    ...
    return (
        <>
            {/* HEADER */}
            <header
                className={`sticky top-0 left-0 w-full z-50 border-b-2 border-yellow-500 transition-all duration-300 
                ${isScrolled ? "bg-black/80 backdrop-blur-md shadow-lg" : "bg-black/95"}`}
            >
            ...
            {/* MOBILE MENU (Hero ustiga chiqadi) */}
            <div
                className={`fixed inset-0 z-40 lg:hidden bg-black/95 backdrop-blur-sm transition-transform transition-opacity duration-300 
    ${isMobileMenuOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0 pointer-events-none"}`}
            >
                <div className="flex flex-col items-center justify-start pt-24 space-y-6 px-6">
                    {navigationLinks.map((link, index) => (
                        <a
                            key={link.name}
                            href={link.href}
                            className="text-white hover:text-[#f3aa01] text-xl sm:text-2xl font-semibold transition-all duration-200"
                            onClick={() => setIsMobileMenuOpen(false)}
                    ...
                    <button
                       ...
                       py-3 px-6 rounded-full shadow-lg transform hover:scale-105 
